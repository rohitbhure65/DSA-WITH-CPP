...
void resizeQueue(struct Queue *Q) {
    int size = Q->capacity;  // Store the current capacity
    Q->capacity = Q->capacity * 2;  // Double the capacity of the queue
    Q->array = (int *)realloc(Q->array, sizeof(int) * Q->capacity);  // Reallocate memory for the array with the new capacity

    if (!Q->array) {  // Check if reallocation failed
        printf("Memory Error");  // Print error message if memory allocation fails
        return;  // Exit the function if reallocation failed
    }

    if (Q->front > Q->rear) {  // Check if the queue is wrapped around
        for (int i = 0; i < Q->front; i++) {  // Loop to rearrange the elements
            Q->array[i + size] = Q->array[i];  // Move the elements from the front part of the array to the new positions
        }
        Q->rear = Q->rear + size;  // Update the rear index
    }
...
void enQueue(struct Queue *Q, int data)
{
    ...
    {
       resizeQueue(Q); // call resizeQueue function
    }
...
int main()
{
    struct Queue *Q = createQueue(3);  // Create a queue with a capacity of 4 elements
